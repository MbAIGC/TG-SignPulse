...
# isort: off
# 以下导入必须在 sqlite monkeypatch 之后执行
...
from tg_signer import __version__ as tg_signer_version  # noqa: E402
...
# 静态前端托管：优先使用配置的 web_dir（默认 <repo>/frontend/dist）。
# Vite 构建产物中哈希资源位于 dist/assets，单独挂载以利用 FastAPI 静态文件处理。
...
async def serve_spa(full_path: str):
    ...
    if settings.frontend_dev_url:
        ...
        parsed_frontend = urlsplit(settings.frontend_dev_url)
