...
def patch_client_methods(
    ...
    *,
    ...
    async def fake_invoke(self, *_args, **_kwargs):
        return None

    async def fake_initialize(self):
        self.is_initialized = True

    # TG-SignPulse client lifecycle is connect()-based: the context manager
    # calls connect() (not start()), then get_me/invoke/initialize handshake.
...
async def test_client_context_manager_reference_counting_and_start_stop(
    ...
    counts, calls connect once for the first entry (no reconnect on nested
    entries), and calls stop after the final exit. Network calls are stubbed.
    """
    import tg_signer.core as core
    ...
    async with client as c1:
        ...
        # nested enter should not connect again
    ...
    assert core._CLIENT_REFS.get(key, 0) == 0
    assert getattr(client, "_fake_started", False) is False

    # connect and stop each called exactly once
...
@pytest.mark.asyncio
async def test_client_context_manager_raises_connection_error_during_start(
    monkeypatch, tmp_path
):
    """TG-SignPulse surfaces invalid sessions instead of swallowing them: a
    connection error during client startup propagates after refcount rollback.
    """
    import tg_signer.core as core
...
async def test_login_bootstrap_is_shared_between_concurrent_workers(
    ...
    # TG-SignPulse client __aenter__ performs an extra get_me handshake
    # (invalid-session detection), so one bootstrap = 2 get_me calls;
    # bootstrap sharing itself is verified by dialogs/session counts == 1.
    assert calls["get_me"] == 2
