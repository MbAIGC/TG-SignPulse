...
- TG-SignPulse 是一个 Telegram 自动化签到管理面板。你可以在网页里管理多个账号，配置自动签到任务，并让任务按固定规则每天自动执行。
...
- **同步上游 tg-signer 0.9.1**：修复 Kurigram 特定动态聊天头像解析异常，保留本项目的任务与面板兼容层
...
- **同步上游 tg-signer 0.9.1**（选择性移植）：修复 Kurigram 解析仅含 emoji markup、
  没有可用视频尺寸的动态聊天头像时触发 `ValueError` 的问题；保留本项目
  `SafeGetForumTopics`、Session、任务运行及面板兼容层
- **任务运行可靠性**：修复账号状态检查重复获取同一账号锁导致任务长期停在
  “正在检查账号状态”的死锁；补充执行阶段日志、WebSocket 初始状态快照、取消与恢复状态保护
- **容器与隐私**：生产镜像显式安装 `tgcrypto`；Uvicorn 访问日志和登录审计仅保存脱敏 IP；
  默认不信任代理头，可信反代场景才可通过 `APP_TRUST_PROXY_HEADERS=1` 开启
- **部署维护**：移除未被 CI 调用的 `docker/GHCR.Dockerfile`，生产镜像统一由根目录
  `Dockerfile` 构建；完整验证达到 243 项后端测试通过、前端构建通过
